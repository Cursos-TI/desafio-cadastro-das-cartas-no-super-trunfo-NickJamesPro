...
{   // Declaração de variáveis para a Carta 1
   ...
   printf("densidade_populacional1 = populacao1 / area1;\n");
    ...
    // Atribuição dos valores com base na opção escolhida
    float valor1, valor2;

    valor1 = (opcao == 1) * populacao1 +
             (opcao == 2) * area1 +
             (opcao == 3) * pib1 +
             (opcao == 4) * pontos_turisticos1 +
             (opcao == 5) * Densidade_populacional1 +
             (opcao == 6) * PIB_per_capita1;

    valor2 = (opcao == 1) * populacao2 +
             (opcao == 2) * area2 +
             (opcao == 3) * pib2 +
             (opcao == 4) * pontos_turisticos2 +
             (opcao == 5) * Densidade_populacional2 +
             (opcao == 6) * PIB_per_capita2;

    // Impressão do resultado usando vetor de strings (sem if/else)
    char *resultado[] = {
        "Carta vencedora: Carta 1\n",
        "Carta vencedora: Carta 2\n",
        "Empate!\n"};

    int indice = (valor1 < valor2) + 2 * (valor1 == valor2); // 0 para C1, 1 para C2, 2 para empate
    printf("%s", resultado[indice]);
